function WhyIta() {
    return (
        <div className="flex justify-center items-center w-[100vw] h-[100vh] dark-text-s light-text-s">
            <div className="flex flex-col justify-center items-center w-[90vw] h-[95vh]">
                <h1 data-aos="fade-up" className="text-3xl poppins-extrabold p-10">Why Choose ITA?</h1>
                <p> dkj</p>
                <div className="services w-[80%] h-[80%] bg-white/10 backdrop-blur-lg shadow-2xl shadow-blue-300 group-[.dark-mode]:shadow-none group-[.dark-mode]:shadow-2xl flex flex-col gap-10 justify-around rounded-2xl">

                </div>
            </div>
        </div>
    )
}

export default WhyIta;